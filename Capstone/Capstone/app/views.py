...
def signup(request):
    if request.method == 'POST':
        ...
        profile = Profile.objects.create(
            user=user,
            user_type='student',  # All signups are students
            student_number=student_number,
        )
...
def counselor_cases(request):
    ...
    try:
        ...
        all_cases = Case.objects.filter(is_active=True).select_related(
            'student__user', 'counselor__user'
        ).order_by('-created_at')
        
        active_cases = all_cases.filter(status__in=['pending', 'in_progress'])
        resolved_cases = all_cases.filter(status='resolved')
        
        pending_count = all_cases.filter(status='pending').count()
        in_progress_count = all_cases.filter(status='in_progress').count()
...
def counselor_reports(request):
    try:
        ...
        # Sessions by month (dummy data for now)
        sessions_by_month = [12, 19, 15, 25, 22, 30]
        ...
        resolved_cases = Case.objects.filter(status='resolved', is_active=True).count()
...
def counselor_reports_export(request, format):
    try:
        ...
        if format == 'excel':
            ...
            w = csv.writer(out)
            w.writerow(['Sessions'])
            w.writerow(['ID','Student','Reason','Status','Scheduled Date'])
            for s in sessions:
                reason = getattr(s, 'get_reason_display')() if hasattr(s, 'get_reason_display') else s.reason
                w.writerow([s.id, s.student.user.get_full_name(), reason, s.status, s.scheduled_date])
            w.writerow([])
            w.writerow(['Cases'])
            w.writerow(['ID','Title','Student','Counselor','Status','Updated'])
            for c in cases:
                w.writerow([c.id, c.title, c.student.user.get_full_name(), c.counselor.user.get_full_name() if c.counselor else '', c.status, c.updated_at])
            resp = HttpResponse(out.getvalue(), content_type='text/csv')
            resp['Content-Disposition'] = 'attachment; filename="cvsu_reports.csv"'
            return resp
        elif format == 'pdf':
            try:
                ...
                p.drawString(50, y, 'CVSU Case Tracking - Summary Report')
                y -= 30
                p.setFont('Helvetica-Bold', 12)
                p.drawString(50, y, 'Sessions')
                y -= 20
                p.setFont('Helvetica', 10)
                for s in sessions[:50]:
                    reason = getattr(s, 'get_reason_display')() if hasattr(s, 'get_reason_display') else s.reason
                    line = f"#{s.id} {s.student.user.get_full_name()} - {reason} - {s.status}"
                ...
                y -= 10
                p.setFont('Helvetica-Bold', 12)
                p.drawString(50, y, 'Cases')
                y -= 20
                p.setFont('Helvetica', 10)
                for c in cases[:50]:
                    line = f"#{c.id} {c.title} - {c.student.user.get_full_name()} - {c.status}"
                    p.drawString(50, y, line[:100])
                    y -= 14
                    if y < 80:
                        p.showPage(); y = height - 50
                p.showPage(); p.save(); buf.seek(0)
                return FileResponse(buf, as_attachment=True, filename='cvsu_reports.pdf')
            except Exception as e:
